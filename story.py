...
# Compile all phone number regex patterns into one list for more efficient use
phone_regexes = [
    re.compile(r'\+?1?\s?[-\(\)\s]*\d{3}[-\)\s]*\d{3}[-\s]*\d{4}'),  # US number variations
    re.compile(r'\+?\d{1,3} \d{3} \d{3} \d{4}'),  # International format 1
    re.compile(r'\d{5}-\d{5}'),  # Another format
    re.compile(r'011-\d{10}'),  # Another international format
    re.compile(r'\+\d{2} \d{5} \d{5}'),  # Yet another
    re.compile(r'\+\d{2} \d{2} \d{10}'),  # And another
    re.compile(r'\d{3} \d{3} \d{4}'),  # Domestic without separators
    re.compile(r'\+\d{2} \d{3} \d{8}'),  # More international
    re.compile(r'\+\d{2} \d{10}')  # And one more for good measure
]
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            for line in tqdm(lines, desc="Progress", unit="lines", bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt}"):
                ...
                # Use spaCy for better name recognition
                doc = nlp(line)
                for entity in doc.ents:
                    if entity.label_ == 'PERSON':
                        name_matches.add(entity.text)
