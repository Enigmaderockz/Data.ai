def generate_html_report(data, table1_html, table2_html, file1_name="File 1", file2_name="File 2", table_border_color="#4a90e2", table_font_family="Arial"):
    ...
    for i, title in enumerate(section_titles):
        ...
        html_content += f"<h2 onclick=\"toggleSection('{section_ids[i]}', this.querySelector('.toggle-icon'))\">"
        html_content += f"<span class='toggle-icon'>{'▼' if default_display == 'block' else '▶'}</span>{title}</h2>"
    ...
    print("HTML report with expandable headings and theme toggle has been generated.")
